...
void pubkey2address(const uint8_t *pubkey, size_t key_length,
                    uint8_t *address) {
  SHA256_CTX ctx;
...
void clearStack() { myStackSize = 0; }

int tx_to_tw_raw(Cosmos__Tx__V1beta1__Tx *tx,
                 Cosmos__Tx__V1beta1__TxRaw *txRaw) {
  txRaw->body_bytes.len =
      cosmos__tx__v1beta1__tx_body__get_packed_size(tx->body);
  ...
  txRaw->auth_info_bytes.len =
      cosmos__tx__v1beta1__auth_info__get_packed_size(tx->auth_info);
  txRaw->auth_info_bytes.data = (uint8_t *)getStack(txRaw->auth_info_bytes.len);
  if (!txRaw->auth_info_bytes.data)
    return -1;
  cosmos__tx__v1beta1__auth_info__pack(tx->auth_info,
                                       txRaw->auth_info_bytes.data);
  return 0;
}

int prepareTx(Google__Protobuf__Any *anyMsg, Cosmos__Base__V1beta1__Coin *coin,
              uint8_t *priv_key, uint8_t *pub_key, uint64_t sequence,
              const char *chain_id, uint64_t account_id, uint8_t **tx_bytes,
              size_t *tx_size) {
  ...
  Cosmos__Tx__V1beta1__ModeInfo__Single mode_single =
      COSMOS__TX__V1BETA1__MODE_INFO__SINGLE__INIT;
  mode_single.mode = COSMOS__TX__SIGNING__V1BETA1__SIGN_MODE__SIGN_MODE_DIRECT;

  Cosmos__Tx__V1beta1__ModeInfo mode_info =
      COSMOS__TX__V1BETA1__MODE_INFO__INIT;
  mode_info.sum_case = COSMOS__TX__V1BETA1__MODE_INFO__SUM_SINGLE;
  mode_info.single = &mode_single;

  Cosmos__Crypto__Secp256k1__PubKey pubkey =
      COSMOS__CRYPTO__SECP256K1__PUB_KEY__INIT;
  pubkey.key.len = 33;
  ...
  any_pub_key.value.len =
      cosmos__crypto__secp256k1__pub_key__get_packed_size(&pubkey);
  ...
  Cosmos__Tx__V1beta1__SignerInfo signInfo =
      COSMOS__TX__V1BETA1__SIGNER_INFO__INIT;
  ...
  Cosmos__Tx__V1beta1__SignerInfo *signer_infos[1] = {
      (Cosmos__Tx__V1beta1__SignerInfo *)&signInfo};
  ...
  int res =
      ecdsa_sign_digest(curve, (const unsigned char *)priv_key,
                        (const unsigned char *)digest, signature, NULL, NULL);
...
int generateAnyAttestMachineMsg(
    Google__Protobuf__Any *anyMsg,
    Planetmintgo__Machine__MsgAttestMachine *machineMsg) {
  anyMsg->type_url = "/planetmintgo.machine.MsgAttestMachine";
  anyMsg->value.len =
      planetmintgo__machine__msg_attest_machine__get_packed_size(machineMsg);
  anyMsg->value.data = getStack(anyMsg->value.len);
  if (!anyMsg->value.data)
    return -1;
  planetmintgo__machine__msg_attest_machine__pack(machineMsg,
                                                  anyMsg->value.data);
  return 0;
}
...
int generateAnyCIDAttestMsg(Google__Protobuf__Any *anyMsg, const char *cid,
  ...
  Planetmintgo__Asset__MsgNotarizeAsset msg =
      PLANETMINTGO__ASSET__MSG_NOTARIZE_ASSET__INIT;
  ...
  anyMsg->value.len =
      planetmintgo__asset__msg_notarize_asset__get_packed_size(&msg);
...
bool get_address_info_from_accounts(const char *json_obj, const char *address,
                                    int *account_id, int *sequence) {
