...
char *create_cid_v1_from_string(const char *data) {
  ...
  uint8_t
      pre_encoded_cid[SHA256_DIGEST_LENGTH + 4]; // 4 extra bytes for version
                                                 // and codec and digest length
  ...
  memcpy(pre_encoded_cid + 4, hash,
         SHA256_DIGEST_LENGTH); // copy the hash after the version and codec

  size_t input_length =
      SHA256_DIGEST_LENGTH +
      4; // calculate the length of the actual content to be encoded
  size_t buffer_size =
      (input_length + 4) / 5 * 8 +
      1; // calculate the buffer size for the base32-encoded string

  char *base32_cid = (char *)malloc(
      buffer_size + 2); // Allocate 2 extra bytes: one for the 'b' prefix and
                        // one for the null terminator
  base32_cid[0] = 'b';  // Add 'b' prefix
  base32_encode(pre_encoded_cid, input_length, base32_cid + 1, buffer_size,
...
uint8_t *decode_cid_v1(const char *base32_cid) {
  size_t input_length =
      strlen(base32_cid) - 1; // Calculate the length of the base32-encoded
                              // string without the 'b' prefix
  size_t buffer_size = (input_length * 5 + 7) /
                       8; // Calculate the buffer size for the decoded string
  ...
  uint8_t *end = base32_decode(base32_cid + 1, input_length, decoded_cid,
                               buffer_size, BASE32_ALPHABET_RFC4648);
  if (!end) {
    free(decoded_cid);
  ...
  if (cid_version != 0x01 || multicodec != 0x55 || sha265_version != 0x12 ||
      digest_length != 0x20) {
    free(decoded_cid);
